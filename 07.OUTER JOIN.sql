...
-- # LEFT OUTER JOIN
-- 조인되는 왼쪽 테이블은 모두 조회하고, 
--오른쪽 테이블은 조인조건에 매칭된 것만 조회합니다.

-- 나는 사원정보는 일단 다 보고 싶은데 
-- 혹시 부서테이블에서 부서코드가 매칭되면
-- 보너스로 부서이름도 찍어줘라
...
SELECT
 A.emp_no, A.emp_nm, A.dept_cd, B.dept_nm
FROM tb_emp A --처음이 왼쪽테이블 (사원정보)
LEFT OUTER JOIN tb_dept B --두번째가 오른쪽 테이블
ON A.dept_cd = B.dept_cd
;
...
SELECT
 A.emp_no, A.emp_nm, A.dept_cd, B.dept_nm
FROM tb_emp A --처음이 왼쪽테이블 (사원정보)
LEFT JOIN tb_dept B --두번째가 오른쪽 테이블
ON A.dept_cd = B.dept_cd
;
...
-- # RIGTH OUTER JOIN
-- 조인되는 오른쪽 테이블은 모두 조회하고, 
-- 왼쪽 테이블은 조인조건에 매칭된 것만 조회합니다.

--ANSI 표준
SELECT
 A.emp_no, A.emp_nm, A.dept_cd, B.dept_nm
FROM tb_emp A --처음이 왼쪽테이블 (사원정보)
RIGHT OUTER JOIN tb_dept B --두번째가 오른쪽 테이블
ON A.dept_cd = B.dept_cd
;
...
SELECT
 A.emp_no, A.emp_nm, A.dept_cd, B.dept_nm
FROM tb_emp A --처음이 왼쪽테이블 (사원정보)
RIGHT JOIN tb_dept B --두번째가 오른쪽 테이블
ON A.dept_cd = B.dept_cd
;
...
SELECT
 A.emp_no, A.emp_nm, A.dept_cd, B.dept_nm
FROM tb_emp A --처음이 왼쪽테이블 (사원정보)
FULL OUTER JOIN tb_dept B --두번째가 오른쪽 테이블
ON A.dept_cd = B.dept_cd
;
...
SELECT
 A.emp_no, A.emp_nm, A.direct_manager_emp_no,B.emp_nm --상사이름
FROM tb_emp A 
LEFT JOIN tb_emp B
