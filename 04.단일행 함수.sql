...
--REPLACE(X, a, b):X에서 a를 찾아 전부 b로 바꿈
...
WHERE sn = '101'
    AND char_4 = char_6
;
--char타입끼리 비교 공백 무시하고 글자만 같으면 같다고 말함
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
    REPLACE(char_6, ' ', '+') AS char_6
FROM char_compare
...
--의도적으로 공백을 넣어야 +가 추가됨/varchar가변문자열
--최대 100글자, 유연함/공백 무시 ㄴㄴ/공백을 유의미하게 봄
--글자 수가 다르면 다르다고 봄
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
...
SELECT 
    ...
    SUBSTR('SQL Developer', 5 , 3), -- 문자열 자름 1번부터 3개자름 (첫글자가 1번)
    LENGTH('HELLO WORLD'),         -- 문자열의 길이/11글자
    TRIM('    HI   ')              -- 좌우 공백 제거
FROM dual;

SELECT 
    RPAD('Steve', 10, '-'), -- 오른쪽에 주어진 문자를 채움/5글자인데 10글자 차지해라
...
SELECT 
    ...
    ROUND(38.678, 3) AS round, -- 자리수까지 반올림
...
SELECT
    direct_manager_emp_no --COUNT(*)
FROM tb_emp
WHERE emp_nm = '박찬호' --'김회장'
;--NULL은 데이터가 있어서 COUNT가 나옴
--NULL 데이터가 있는데 아직 미정
--NULL 다른 값으로 바꿀 수 있음
SELECT 
